...
//**********************************************************************************************			
			//El compareTo se modifica para que la clase de heapsort ordene al revez los numeros
	...
	public void ahiFueQue(Integer[] choco, Integer[] arequi,BinaryHeap<Nodo<Integer>> torta, BinaryHeap<Valor<Integer>> chocolate, BinaryHeap<Valor<Integer>> arequipe,int niveles){
	...
	for(int i=0;i<niveles-1;i++){
		...
		for (int j=0;j<niveles-1;j++){
			
			if (((choco[i])+arequi[j])<(choco[i+1]+arequi[i+1])){
				
				for (int k=i+2;k<niveles-1;k++){
					
				if (((choco[i])+arequi[j])<=(choco[i+1]+arequi[k])){
			...
			Nodo<Integer> leviosa=new Nodo<Integer>(choco[i], arequi[j]);
			torta.insert(leviosa);	
		
		}
			
		 for (int j=0;j<niveles-1;j++){
			if (((choco[j])+arequi[i])<(choco[i+1]+arequi[i+1])){
				for (int k=i+2;k<niveles-1;k++){
	...
	Integer[] choc=new Integer[niveles];//arreglo para almacenar niveles de chocolate y arequipe
    Integer[] arequip=new Integer[niveles];
...
//	if (niveles<parejas){
	...
	//torta.insert(myNodo);		
	  ...
	  pb.ahiFueQue(choc, arequip, torta, chocolate, arequipe, niveles);
